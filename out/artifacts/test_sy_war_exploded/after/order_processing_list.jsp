...
//jquery  confirm组件弹窗按钮样式
    ...
    function selectStatus(obj, id) {
        var name = $(obj).text();
        $("#dropdownMenuGetValue2").val(name);
        $("#orderStatus").val(id);
        <%--//var stuids=document.getElementById("show").value.innerHTML=${goods.goodstype.typename};--%>
    }
            ...
            <form action="<%=path %>/OrderFuzzySelectServlet" class="form-inline" id="formForOrder">
                <div class="form-group">
                    <div class="input-group" style="width: 220px;">
                        ...
                        <input type="text" name="username" class="form-control" style="width: 220px;"
                               id="exampleInputName" placeholder="请输入名字"/>
                    </div>
                </div>
                ...
                <div class="form-group" style="margin-left: 20px;">
                    <div class="input-group" style="width: 220px;">
                        ...
                        <div class="dropdown">
                            <input class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuGetValue2"
                                   data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" value="-------"
                                   style="min-width: 135px;text-align: left;background-color: white;color: royalblue;">
                            <span class="caret" style="position: absolute;top: 16px;right: 10px;"></span>
                ...
                <button type="submit" style="float: right;" onclick="checkValue02()" class="btn btn-primary">
                ...
                <tr>
                    ...
                    <td>
                        <a class="glyphicon glyphicon-trash" color="#5BC0DE" style="text-decoration: none"
                           name="ordersid" href="<%=path%>/DeleteOrderServlet?ordersid=${orders.id}"
                           onclick="if(confirm('是否确定删除'+'${orders.orderCode}'+'？')==false)return false">
                        </a>
                    </td>
                </tr>
            </c:forEach>
...
<div class="container">
    <nav aria-label="..." class="navbar-right" style="margin-right: 15px">
        <ul class="pagination">
            <li>
                <a class=" btn-info" href="<%=path %>/OrderFuzzySelectServlet?pageNo=1" aria-label="Previous">
            ...
            <li class="disabled"><a href="#" aria-label="Previous">« <span class="sr-only">(current) </span></a></li>
            <%
                }
                if (pageCount > pageNo) {
            %>

            <li><a class="btn" href="<%=path %>/OrderFuzzySelectServlet?pageNo=${pageNo+1}">${pageNo}</a></li>
            <li>
                <a class="btn-info"
            ...
            %>
            <li>
                <a class=" btn-info" href="<%=path %>/OrderFuzzySelectServlet?pageNo=${pageCount}"
                   aria-label="Previous"> <span aria-hidden="true">尾页 </span></a>
            </li>
        </ul>
    </nav>
</div>
